...
const EmptyStatePlaceholder = ({ text }: { text: string }) => (
    <div className="bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-xl flex flex-col items-center justify-center text-center text-zinc-400 dark:text-zinc-500 p-8 max-w-lg w-full shadow-sm mx-auto my-auto">
            ...
            <div key={attribute.name} className="grid grid-cols-[100px_1fr] sm:grid-cols-[120px_1fr] gap-4 items-center py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors px-2 rounded-lg">
                ...
                <div className={`flex rounded-md border overflow-hidden w-full ${isModified ? 'border-blue-500 ring-1 ring-blue-200 dark:ring-blue-900' : 'border-zinc-300 dark:border-zinc-700'}`}>
                     <button
                        ...
                        className={`w-1/2 text-xs font-semibold py-1.5 px-2 transition-colors truncate disabled:cursor-not-allowed disabled:bg-zinc-200 disabled:text-zinc-500 ${
                            isPresent
                ...
                {isCustomMode ? (
                    <div className="flex gap-1">
                        <input
                            ...
                            className="w-full bg-white dark:bg-zinc-900 border border-blue-500 text-zinc-800 dark:text-zinc-200 text-sm rounded px-2 py-1.5 focus:outline-none shadow-sm"
                            onKeyDown={(e) => {
                        ...
                        <button
                            ...
                            className="bg-blue-600 text-white hover:bg-blue-700 text-xs font-semibold px-2 rounded disabled:bg-zinc-300"
                        >
                            Save
                        </button>
                        <button
                    ...
                    <div className="relative">
                        <select 
                            ...
                            disabled={isLoading} 
                            className={`w-full text-zinc-800 dark:text-zinc-200 text-sm rounded-md shadow-sm px-3 py-2 pr-8 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent cursor-pointer transition-colors ${
                                isModified 
                                ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 border' 
                                : 'border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 border hover:border-zinc-400 dark:hover:border-zinc-600'
                            }`}
                        >
                            ...
                            <option value="__custom__" className="font-bold text-blue-600 dark:text-blue-400 bg-zinc-50 dark:bg-zinc-800">
                                + Type custom...
                            </option>
                        </select>
...
const RelationshipEditor: React.FC<{
  ...
  return (
    <div
      ref={editorRef}
      className="absolute bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-xl z-40 w-64 max-h-[300px] overflow-y-auto flex flex-col"
      style={adjustedStyle}
      onPointerDown={(e) => e.stopPropagation()}
      onWheel={(e) => e.stopPropagation()}
    >
      <div className="flex justify-between items-center mb-2 flex-shrink-0">
          <h4 className="font-bold text-sm text-zinc-800 dark:text-zinc-100">Edit Relationship</h4>
      ...
      <div className="flex flex-col space-y-1 mb-3 flex-grow">
        {allOptions.map(alt => (
            <button
                ...
                className={`text-left text-sm p-1 px-2 rounded-lg transition-colors w-full ${rel.label === alt.name ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-100 font-semibold' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300'} disabled:opacity-50`}
            >
                {alt.name}
            </button>
        ))}
      </div>
      <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800 flex-shrink-0">
          <div className="flex gap-2">
              <input 
                  ...
                  className="flex-grow text-xs border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 rounded-lg p-1.5 focus:outline-none focus:border-blue-500"
                  disabled={isLoading}
              />
              <button 
                  onClick={handleCustomSubmit}
                  disabled={isLoading || !customLabel.trim()}
                  className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-2 rounded-lg disabled:bg-zinc-300"
              >
                  Save
              </button>
          </div>
...
const BeliefGraph: React.FC<BeliefGraphProps> = ({ 
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 w-full flex flex-col flex-1 min-h-0 relative transition-colors duration-200 shadow-sm">
      
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-sm flex items-center justify-center z-50 rounded-xl transition-opacity duration-300">
      ...
      {/* Attributes View */}
      <div style={{ display: view === 'attributes' ? 'flex' : 'none' }} className="flex-col overflow-y-auto flex-grow w-full h-full bg-white dark:bg-zinc-900 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700 rounded-xl">
      ...
      >
        <div 
          className={`relative bg-zinc-50/50 dark:bg-zinc-950/50 w-full h-full overflow-hidden rounded-xl ${!data && !isLoading ? 'flex items-center justify-center p-6' : ''}`} 
          ref={measureRef}
        >
            {data && (
                <div className="absolute top-4 left-4 flex flex-col gap-2 z-40">
                    <button onClick={zoomIn} className="bg-white dark:bg-zinc-800 p-2 rounded-lg shadow text-zinc-600 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 focus:outline-none" title="Zoom In">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clipRule="evenodd" /></svg>
                    </button>
                    <button onClick={zoomOut} className="bg-white dark:bg-zinc-800 p-2 rounded-lg shadow text-zinc-600 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 focus:outline-none" title="Zoom Out">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" clipRule="evenodd" /></svg>
                    </button>
                    <button onClick={resetView} className="bg-white dark:bg-zinc-800 p-2 rounded-lg shadow text-zinc-600 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 focus:outline-none" title="Reset View">
            ...
            {data && objectEntities.length === 0 && !isLoading && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center text-zinc-500 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm p-4 rounded-xl pointer-events-auto">
        ...
        {selectedEntity && (
            <div 
            className={`absolute bottom-4 left-4 right-4 md:w-96 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-xl animate-fade-in z-[100] flex flex-col overflow-hidden ${cardStyle ? '' : 'md:right-4 md:top-4 md:bottom-auto md:left-auto md:max-h-[60vh]'}`}
            ...
            <button 
                ...
                className="absolute top-3 right-3 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 z-10 bg-white/50 dark:bg-zinc-800/50 rounded-full p-1"
                aria-label="Close"
            >
            ...
            {/* Scrollable Area */}
            <div className="overflow-y-auto p-4 pt-5 min-h-0 flex-grow scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700">
                <div className="pr-8 mb-2"> {/* Right padding for close button */}
                    <h3 className="font-bold text-lg text-zinc-800 dark:text-zinc-100 leading-tight">Entity: {selectedEntity.name}</h3>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 italic mt-1">{selectedEntity.description}</p>
                </div>
                
                <div className="border-b border-zinc-100 dark:border-zinc-800 mb-3"></div>
