...
interface OutputDisplayProps {
  ...
  onSelectKey?: () => void;
  onSendToMagicPixels?: (image: string) => void;
}
      ...
      if (images.length > 0) {
          const content = images.map((image, index) => (
              ...
              >
                  ...
                  {/* Edit in Magic Pixels button */}
                  {onSendToMagicPixels && (
                    <button
                      onClick={e => { e.stopPropagation(); onSendToMagicPixels(image); }}
                      className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-center gap-1.5 px-2.5 py-1.5 bg-gradient-to-r from-purple-600 to-blue-600 text-white text-xs font-semibold rounded-lg shadow-lg hover:from-purple-500 hover:to-blue-500 focus:outline-none focus:ring-2 focus:ring-purple-400 translate-y-1 group-hover:translate-y-0"
                      title={t.editInMagicPixels}
                    >
                      ...
                      <span className="hidden sm:inline">{t.editInMagicPixels}</span>
                    </button>
                  )}
              </div>
          ));
