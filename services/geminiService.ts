...
// Global instance for standard calls. We initialize it lazily or with a dummy key if not present,
// but we'll recreate it when needed if the key changes.
let ai = new GoogleGenAI({ apiKey: getApiKey() || 'dummy_key' });
...
export const generateVideosFromPrompt = async (prompt: string, onStatusUpdate?: StatusUpdateCallback, image?: AttachedImage | null): Promise<string> => {
    ...
    try {
        ...
        if (!currentKey) throw new Error("API key is required for video generation");
        ...
        let operation = await freshAi.models.generateVideos(videoConfig);
